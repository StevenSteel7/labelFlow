...
from PyQt6.QtGui import (QPixmap, QColor, QIcon, QImage, QFont, QKeySequence, QPalette, QAction,
                         QCursor) # QAction moved here, QCursor added for clarity
...
# Local Imports (Assuming they are or will be PyQt6 compatible)
...
# Shapely imports remain the same
    ...
    def __init__(self, shape, file_name, parent=None, default_dimensions=None):
        ...
        for i, dim in enumerate(shape):
            ...
            if default_dimensions and i < len(default_dimensions):
                # Find index for the default dimension text
                index = combo.findText(default_dimensions[i])
                if index >= 0:
        ...
        self.button.clicked.connect(self.accept) # Use accept directly
...
class ImageAnnotator(QMainWindow):
        ...
        if os.path.exists(project_file):
            try:
                ...
                if self.project_creation_date:
                     ...
                     except ValueError: # Handle cases where the format might be slightly different
                         self.project_creation_date = str(self.project_creation_date)
    ...
    def load_missing_images(self, missing_images):
        # Use the new static method style for QFileDialog
    ...
    def activate_sam_magic_wand(self):
        ...
        self.image_label.setCursor(QCursor(Qt.CursorShape.CrossCursor)) # Use QCursor and enum
    ...
    def deactivate_sam_magic_wand(self):
        ...
        self.image_label.setCursor(QCursor(Qt.CursorShape.ArrowCursor)) # Use QCursor and enum
    ...
    # qimage_to_numpy function is removed from here as it relies on PyQt5's QImage.bits()
    # It needs to be rewritten in sam_utils.py using PyQt6's QImage data access methods.
    # def qimage_to_numpy(self, qimage):
    #     # ... (PyQt5 implementation) ...
    #     # THIS FUNCTION NEEDS REWRITING FOR PyQt6
    #     pass
        ...
        for file_name in file_names:
            ...
            if base_name not in self.image_paths:
                ...
                if file_name.lower().endswith(('.tif', '.tiff', '.czi')):
                    ...
                    if not image.isNull(): # Check if image loaded successfully
                        image_info["height"] = image.height()
                        image_info["width"] = image.width()
                    else:
                         print(f"Warning: Could not load image {file_name}")
                         # Optionally skip this file or set default dimensions
                         # continue
    ...
    def closeEvent(self, event):
        if not self.image_label.check_unsaved_changes():
            event.ignore()
            return
        # event.accept() # Accept is called later if checks pass
    ...
    def activate_current_slice(self):
        if self.current_slice:
            ...
            items = self.slice_list.findItems(self.current_slice, Qt.MatchFlag.MatchExactly) # Use MatchFlag
            if items:
        ...
        if dimensions is None or force_dimension_dialog:
            if image_array.ndim > 2:
                ...
                while True:
                    ...
                    # Remove help button flag (syntax might change slightly based on exact PyQt6 version/platform)
                    dialog.setWindowFlags(dialog.windowFlags() & ~Qt.WindowType.WindowContextHelpButtonHint) # Use enum
                    progress.setValue(50)
                    QApplication.processEvents()
                    if dialog.exec(): # Use exec()
    ...
    def create_slices(self, image_array, dimensions, image_path):
        ...
        if image_array.ndim == 2:
            ...
            for idx, _ in enumerate(np.ndindex(tuple(image_array.shape[i] for i in slice_indices))):
                ...
                # Update progress
                progress_value = int((idx + 1) / total_slices * 100) if total_slices > 0 else 100
    ...
    def add_slice_to_list(self, slice_name):
        ...
        # Color setting logic remains the same, QColor usage is compatible
        if self.dark_mode:
            # Dark mode
    ...
    def normalize_array(self, array):
       ...
       # print(f"Array min: {array.min()}, max: {array.max()}, mean: {array.mean()}")
        ...
        max_val = array.max()
        if max_val == min_val:
            array_normalized = np.zeros_like(array_float)
        elif array.dtype == np.uint16:
            array_normalized = (array_float - min_val) / (max_val - min_val)
            ...
            if p_high == p_low: # Handle flat image case
                array_normalized = np.zeros_like(array_float)
            else:
    ...
    def adjust_contrast(self, image, low_percentile=1, high_percentile=99):
        if image.dtype != np.uint8:
            ...
            if p_high == p_low: # Handle flat image
    ...
    def activate_slice(self, slice_name):
        ...
        items = self.slice_list.findItems(slice_name, Qt.MatchFlag.MatchExactly) # Use MatchFlag
        if items:
            self.slice_list.setCurrentItem(items[0])
    ...
    def array_to_qimage(self, array):
        ...
        if array.ndim == 2: # Grayscale
             # Ensure it's uint8 for Grayscale8
             if array.dtype != np.uint8:
                 array = self.normalize_array(array) # Normalize if not already 8-bit
             return QImage(array.data, width, height, bytes_per_line, QImage.Format.Format_Grayscale8)
        elif array.ndim == 3:
             if array.shape[2] == 3: # RGB
                 # Ensure it's uint8 for RGB888
                 if array.dtype != np.uint8:
                      array = self.normalize_array(array) # Normalize if not already 8-bit
                 return QImage(array.data, width, height, bytes_per_line, QImage.Format.Format_RGB888)
             elif array.shape[2] == 4: # RGBA
                  # Ensure it's uint8 for ARGB32
                  if array.dtype != np.uint8:
                       array = self.normalize_array(array) # Normalize if not already 8-bit
                  return QImage(array.data, width, height, bytes_per_line, QImage.Format.Format_ARGB32)

        raise ValueError(f"Unsupported array shape {array.shape} or dtype {array.dtype} for conversion to QImage")
    ...
    def update_slice_list(self):
        ...
        for slice_name, _ in self.slices:
            ...
            if slice_name in self.all_annotations:
                item.setForeground(QColor(Qt.GlobalColor.green)) # Use GlobalColor
            else:
                item.setForeground(QColor(Qt.GlobalColor.black) if not self.dark_mode else QColor(Qt.GlobalColor.white)) # Use GlobalColor
            self.slice_list.addItem(item)
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_F2: # Use Qt.Key enum
            self.launch_snake_game()
        elif event.key() == Qt.Key.Key_Delete: # Use Qt.Key enum
            # Handle deletions
        ...
        elif event.key() == Qt.Key.Key_Up or event.key() == Qt.Key.Key_Down: # Use Qt.Key enum
    ...
    def has_visible_temp_classes(self):
        for i in range(self.class_list.count()):
            ...
            if item.text().startswith("Temp-") and item.checkState() == Qt.CheckState.Checked: # Use enum
                return True
        return False
    ...
    def import_annotations(self):
        ...
        for info in image_info.values():
            ...
            # Use path from image_info directly if available
            image_path = info.get('path', os.path.join(images_dir, info['file_name']))
        ...
        if images_not_found:
            message = f"The following {len(images_not_found)} images were not found in the expected directory:\n\n"
            ...
            if reply == QMessageBox.StandardButton.No:
                ...
                QMessageBox.information(self, "Import Cancelled",
                                        "Import cancelled. Please ensure all images are in the expected directory and try again.")
                return
        ...
        for annotations in self.all_annotations.values():
            for category_name in annotations.keys():
                if category_name not in self.class_mapping:
                    ...
                    # Use Qt.GlobalColor.<color_name>
                    # Choose color based on ID modulo available colors
                    color_index = (new_id % 16) + 7 # Map to Qt.GlobalColor indices 7-22
                    self.image_label.class_colors[category_name] = QColor(Qt.GlobalColor(color_index))
        ...
        message += f"{images_loaded} images were loaded from the expected directory.\n"
        if images_not_found:
    ...
    def export_annotations(self):
        ...
        if export_format == "COCO JSON":
            # QFileDialog.getSaveFileName returns tuple (fileName, selectedFilter)
        ...
        # --- The rest of the export logic remains largely the same ---
        # --- as it deals with data manipulation and file I/O, not UI ---
        if export_format == "COCO JSON":
            ...
            output_dir_exp, yaml_path = export_yolo_v5plus(self.all_annotations, self.class_mapping,
                                                     self.image_paths, self.slices, self.image_slices, file_name)
            message = "Annotations have been exported successfully in YOLO (v5+) format.\n"
            message += f"Output directory: {output_dir_exp}\nYAML: {yaml_path}"
    ...
    def create_coco_annotation(self, ann, image_id, annotation_id):
        ...
        if "segmentation" in ann:
            # COCO expects list of lists for segmentation
    ...
    def update_annotation_list(self, image_name=None):
        ...
        for class_name, class_annotations in annotations.items():
            if not class_name.startswith("Temp-"):  # Only show non-temporary annotations
                color = self.image_label.class_colors.get(class_name, QColor(Qt.GlobalColor.white)) # Use enum
                for annotation in class_annotations:
                    number = annotation.get('number', 0)
                    area = calculate_area(annotation)
                    item_text = f"{class_name} - {number:<3} Area: {area:.2f}"
                    item = QListWidgetItem(item_text)
                    item.setData(Qt.ItemDataRole.UserRole, annotation) # Use enum
    ...
    def update_annotation_list_colors(self, class_name=None, color=None):
        for i in range(self.annotation_list.count()):
            ...
            annotation = item.data(Qt.ItemDataRole.UserRole) # Use enum
            ...
            if class_name is None or annotation['category_name'] == class_name:
                item_color = color if class_name else self.image_label.class_colors.get(annotation['category_name'], QColor(Qt.GlobalColor.white)) # Use enum
    ...
    def setup_class_list(self):
        ...
        self.class_list.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu) # Use enum
    ...
    def setup_tool_buttons(self):
        ...
        manual_label.setAlignment(Qt.AlignmentFlag.AlignCenter) # Use enum
        ...
        automated_label.setAlignment(Qt.AlignmentFlag.AlignCenter) # Use enum
        automated_layout.addWidget(automated_label)

        automated_buttons_layout = QHBoxLayout()
        self.sam_magic_wand_button = QPushButton("Magic Wand") # Reassigned later in setup_sidebar
        self.sam_magic_wand_button.setCheckable(True)
        ...
        # Note: This setup is slightly redundant with setup_sidebar, ensure alignment
        # self.sidebar_layout.addWidget(manual_tools_widget) # Added in setup_sidebar
        # self.sidebar_layout.addWidget(automated_tools_widget) # Added in setup_sidebar


        # Set a fixed size for all buttons to make them smaller
        # Buttons are redefined/added in setup_sidebar, apply size there if needed
        # for button in [self.polygon_button, self.rectangle_button, self.sam_magic_wand_button]:
        #     if hasattr(self, 'load_sam2_button'): # Check if this button exists from original code
        #          button.setFixedSize(100, 30)
        #     elif button != self.sam_magic_wand_button: # Avoid resizing the one added here
        #          button.setFixedSize(100, 30)
    ...
    def setup_annotation_list(self):
        ...
        self.annotation_list.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection) # Use enum
    ...
    def create_menu_bar(self):
        ...
        # QAction is imported from PyQt6.QtGui
        new_project_action = QAction("&New Project", self)
        new_project_action.setShortcut(QKeySequence(QKeySequence.StandardKey.New)) # Use enum
        new_project_action.triggered.connect(self.new_project)
        project_menu.addAction(new_project_action)

        open_project_action = QAction("&Open Project", self)
        open_project_action.setShortcut(QKeySequence(QKeySequence.StandardKey.Open)) # Use enum
        open_project_action.triggered.connect(self.open_project)
        project_menu.addAction(open_project_action)

        save_project_action = QAction("&Save Project", self)
        save_project_action.setShortcut(QKeySequence(QKeySequence.StandardKey.Save)) # Use enum
        ...
        help_action.setShortcut(QKeySequence(QKeySequence.StandardKey.HelpContents)) # Use enum
    ...
    def setup_sidebar(self):
        ...
        def create_section_header(text):
            ...
            label.setAlignment(Qt.AlignmentFlag.AlignLeft) # Use enum
            return label
    ...
    def setup_font_size_selector(self):
        ...
        # Find index for the default font size text
        index = self.font_size_selector.findText("Medium")
        if index >= 0:
            self.font_size_selector.setCurrentIndex(index)
    ...
    def setup_image_area(self):
        ...
        self.scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded) # Use enum
        self.scroll_area.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded) # Use enum

        # Use the already initialized image_label
        self.image_label.setAlignment(Qt.AlignmentFlag.AlignCenter) # Use enum
        self.scroll_area.setWidget(self.image_label)

        self.image_layout.addWidget(self.scroll_area)


        self.zoom_slider = QSlider(Qt.Orientation.Horizontal) # Use enum
        self.zoom_slider.setMinimum(10)
        self.zoom_slider.setMaximum(500)
        self.zoom_slider.setValue(100)
        self.zoom_slider.setTickPosition(QSlider.TickPosition.TicksBelow) # Use enum
    ...
    def setup_image_list(self):
        ...
        self.image_list.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu) # Use enum
...
##########    ### Tools  ########## I love useful image processing tools :)
    def open_dataset_splitter(self):
        ...
        self.dataset_splitter.setWindowModality(Qt.WindowModality.ApplicationModal) # Use enum
...
###################################################################
    ...
    def clear_all(self, new_project=False, show_messages=True):
        ...
        self.image_label.setCursor(QCursor(Qt.CursorShape.ArrowCursor)) # Use QCursor and enum
        # Find index for the default SAM model text
        index = self.sam_model_selector.findText("Pick a SAM Model")
        if index >= 0:
            self.sam_model_selector.setCurrentIndex(index)
    ...
    def show_question(self, title, message):
        return QMessageBox.question(self, title, message,
                                    ...
                                    QMessageBox.StandardButton.No) # Use enum
    ...
    def show_image_context_menu(self, position):
        ...
        if current_item:
            ...
            delete_action = menu.addAction("Remove Image") # QAction from QtGui

            predict_action = None # Initialize to None
            redefine_dimensions_action = None # Initialize to None

            if not self.is_multi_dimensional(file_name):
                predict_action = menu.addAction("Predict using YOLO") # QAction from QtGui

            if self.is_multi_dimensional(file_name):
                redefine_dimensions_action = menu.addAction("Redefine Dimensions") # QAction from QtGui

            action = menu.exec(self.image_list.mapToGlobal(position)) # Use exec()

            if action == delete_action:
                self.remove_image()
            elif predict_action and action == predict_action: # Check if predict_action was created
                self.predict_single_image(file_name)
            elif redefine_dimensions_action and action == redefine_dimensions_action: # Check if redefine_action was created
    ...
    def redefine_dimensions(self, file_name):
        ...
        reply = QMessageBox.warning(self, "Redefine Dimensions",
                                    ...
                                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) # Use enum
        ...
        if file_name:
            ...
            for category in self.loaded_json["categories"]:
                ...
                if class_name not in self.image_label.class_colors:
                    # Use Qt.GlobalColor.<color_name>
                    color_index = (len(self.image_label.class_colors) % 16) + 7
                ...
                if file_name:
                    ...
                    if category:
                        ...
                        if "segmentation" in annotation and annotation["segmentation"]:
                            # Assume COCO format [ [poly1], [poly2]...] or [poly]
                            if isinstance(annotation["segmentation"][0], list):
                                 ann["segmentation"] = annotation["segmentation"][0] # Take first polygon
                            else:
                                 ann["segmentation"] = annotation["segmentation"]
                            ann["type"] = "polygon"
                        elif "bbox" in annotation:
                            ann["bbox"] = annotation["bbox"]
                            ann["type"] = "bbox" # Changed from "rectangle" for consistency? Check type usage.
            ...
            if self.image_file_name and self.image_file_name in self.all_annotations:
                 items = self.image_list.findItems(self.image_file_name, Qt.MatchFlag.MatchExactly) # Use MatchFlag
                 if items:
    ...
    def update_highlighted_annotations(self):
        ...
        self.image_label.highlighted_annotations = [item.data(Qt.ItemDataRole.UserRole) for item in selected_items] # Use enum
        self.image_label.update()  # Force a redraw of the image label
    ...
    def delete_selected_annotations(self):
        ...
        if reply == QMessageBox.StandardButton.Yes: # Use enum
            ...
            keys_to_del = [k for k, v in self.image_label.annotations.items() if not v]
            for k in keys_to_del:
                del self.image_label.annotations[k]

            # Update all_annotations
            current_name = self.current_slice or self.image_file_name
            self.all_annotations[current_name] = self.image_label.annotations.copy() # Save the modified dict
    ...
    def merge_annotations(self):
        ...
        class_name = selected_items[0].data(Qt.ItemDataRole.UserRole)['category_name'] # Use enum
        if not all(item.data(Qt.ItemDataRole.UserRole)['category_name'] == class_name for item in selected_items): # Use enum
        ...
        for item in selected_items:
            ...
            if 'segmentation' in annotation:
                 # Ensure segmentation is treated as a flat list of coordinates
                 if isinstance(annotation['segmentation'], list) and annotation['segmentation']:
                     if isinstance(annotation['segmentation'][0], list):
                         # Handle COCO format [ [poly] ] by taking the first polygon
                         flat_segmentation = annotation['segmentation'][0]
                     else:
                         flat_segmentation = annotation['segmentation']

                     points = zip(flat_segmentation[0::2], flat_segmentation[1::2])
                     polygon = Polygon(points)
                     if not polygon.is_valid:
                         polygon = polygon.buffer(0) # Try to fix invalid geometry
                     polygons.append(polygon)

        if not polygons:
             QMessageBox.warning(self, "No Polygons", "Selected annotations do not contain valid polygon data.")
             return
        ...
        new_annotation = {
            ...
            "type": "polygon" # Merged result is a polygon
        }

        # Ensure segmentation is a flat list
        if isinstance(merged_polygon, Polygon):
            new_annotation["segmentation"] = [coord for point in merged_polygon.exterior.coords for coord in point][:-2] # Remove closing point duplication
        elif isinstance(merged_polygon, MultiPolygon):
            largest_polygon = max(merged_polygon.geoms, key=lambda p: p.area)
            new_annotation["segmentation"] = [coord for point in largest_polygon.exterior.coords for coord in point][:-2] # Remove closing point duplication
        else:
             QMessageBox.warning(self, "Merge Error", "Merged result is not a valid polygon or multipolygon.")
             return
        ...
        msg_box.setIcon(QMessageBox.Icon.Question) # Use enum

        keep_button = msg_box.addButton("Keep", QMessageBox.ButtonRole.YesRole) # Use enum
        delete_button = msg_box.addButton("Delete", QMessageBox.ButtonRole.NoRole) # Use enum
        cancel_button = msg_box.addButton("Cancel", QMessageBox.ButtonRole.RejectRole) # Use enum
        ...
        current_name = self.current_slice or self.image_file_name
        self.all_annotations[current_name] = self.image_label.annotations.copy() # Update all annotations
    ...
    def display_image(self):
        if self.current_image:
            ...
            if not pixmap.isNull():
                ...
                # self.image_label.adjustSize() # adjustSize might conflict with scroll area
                self.image_label.setMinimumSize(self.image_label.scaled_pixmap.size()) # Use scaled size for min size
            else:
    ...
    def add_class(self, class_name=None, color=None):
        ...
        if color is None:
             # Use Qt.GlobalColor.<color_name>
             color_index = (len(self.image_label.class_colors) % 16) + 7 # Map to Qt.GlobalColor indices 7-22
             color = QColor(Qt.GlobalColor(color_index))
        ...
        try:
            ...
            item.setData(Qt.ItemDataRole.UserRole, True) # Use enum

            # Set checkbox
            item.setFlags(item.flags() | Qt.ItemFlag.ItemIsUserCheckable) # Use enum
            item.setCheckState(Qt.CheckState.Checked) # Use enum
    ...
    def update_class_list(self):
        ...
        for class_name, color in self.image_label.class_colors.items():
            ...
            item.setData(Qt.ItemDataRole.UserRole, self.image_label.class_visibility.get(class_name, True)) # Use enum

            # Set checkbox
            item.setFlags(item.flags() | Qt.ItemFlag.ItemIsUserCheckable) # Use enum
            item.setCheckState(Qt.CheckState.Checked if item.data(Qt.ItemDataRole.UserRole) else Qt.CheckState.Unchecked) # Use enum

            self.class_list.addItem(item)

        # Re-select the current class if it exists
        if self.current_class:
            items = self.class_list.findItems(self.current_class, Qt.MatchFlag.MatchExactly) # Use enum
            if items:
    ...
    def toggle_class_visibility(self, item):
        ...
        is_visible = item.checkState() == Qt.CheckState.Checked # Use enum
        self.image_label.set_class_visibility(class_name, is_visible)
        item.setData(Qt.ItemDataRole.UserRole, is_visible) # Use enum
        self.image_label.update()
    ...
    def change_annotation_class(self):
        ...
        if class_dialog.exec() == QDialog.DialogCode.Accepted: # Use enum
            ...
            for item in selected_items:
                ...
                if old_class in self.image_label.annotations: # Check if class exists
                    if annotation in self.image_label.annotations[old_class]:
            ...
            # Update all_annotations
            self.all_annotations[current_name] = self.image_label.annotations.copy() # Copy updated dict
    ...
    def toggle_tool(self):
        ...
        sender = self.sender()
        if sender is None:
            sender = self.sam_magic_wand_button # Fallback, should ideally not happen
    ...
    def wheelEvent(self, event):
        # Use globalPos() and angleDelta() for PyQt6 compatibility if needed, but pos() usually works
        if event.modifiers() == Qt.KeyboardModifier.ControlModifier: # Use enum
            ...
            if self.image_label.current_tool == "paint_brush":
                ...
                self.image_label.update() # Update to show new cursor size
    ...
    def update_ui_for_current_tool(self):
        ...
        # This button seems removed/integrated elsewhere in the provided code
        ...
        for button in self.tool_group.buttons():
             # Special handling for SAM button which depends on model selection too
             if button == self.sam_magic_wand_button:
                 button.setEnabled(tools_enabled and self.current_sam_model is not None)
             else:
        ...
        if self.image_label.current_tool == "sam_magic_wand" and self.sam_magic_wand_button.isEnabled():
            ...
            # Reset to arrow unless another tool sets a specific cursor (like paint/eraser might)
             if self.image_label.current_tool not in ["paint_brush", "eraser"]: # Add other tools if they set cursors
                 self.image_label.setCursor(QCursor(Qt.CursorShape.ArrowCursor)) # Use QCursor and enum

    def on_class_selected(self, current=None, previous=None): # Signature change in itemClicked
        if not self.image_label.check_unsaved_changes():
            return

        # In PyQt6, itemClicked often passes the QListWidgetItem directly
        if current is None:
            current = self.class_list.currentItem()
        elif isinstance(current, QListWidgetItem): # Check if it's the item
             pass # Already have the item
        else: # If it's not the item (e.g., from manual call), get current item
             current = self.class_list.currentItem()
    ...
    def enable_annotation_tools(self):
        for button in self.tool_group.buttons():
             # Special handling for SAM button
             if button == self.sam_magic_wand_button:
                 button.setEnabled(self.current_sam_model is not None)
             else:
                 button.setEnabled(True)

    def show_class_context_menu(self, position):
        menu = QMenu()
        rename_action = menu.addAction("Rename Class") # QAction from QtGui
        change_color_action = menu.addAction("Change Color") # QAction from QtGui
        delete_action = menu.addAction("Delete Class") # QAction from QtGui

        item = self.class_list.itemAt(position)
        if item:
            action = menu.exec(self.class_list.mapToGlobal(position)) # Use exec()
        ...
        # else: # Removed redundant warning - context menu shouldn't show if no item
        #     QMessageBox.warning(self, "No Selection", "Please select a class to perform actions.")

    def change_class_color(self, item):
        class_name = item.text()
        current_color = self.image_label.class_colors.get(class_name, QColor(Qt.GlobalColor.white)) # Use enum
        # QColorDialog.getColor returns a QColor
    ...
    def finish_polygon(self):
        if self.image_label.current_tool == "polygon" and len(self.image_label.current_annotation) > 2:
            ...
            new_annotation = {
                ...
                "category_id": self.class_mapping[self.current_class],
                "category_name": self.current_class,
                "type": "polygon" # Explicitly set type
            }
    ...
    def highlight_annotation(self, item):
        self.image_label.highlighted_annotation = item.data(Qt.ItemDataRole.UserRole) # Use enum
        self.image_label.update()

    def delete_annotation(self):
        current_item = self.annotation_list.currentItem()
        if current_item:
            annotation = current_item.data(Qt.ItemDataRole.UserRole) # Use enum
    ...
    def add_annotation_to_list(self, annotation):
        ...
        # Ensure 'number' exists, assign if not
        if 'number' not in annotation:
            # Calculate the next number based on existing annotations for this class
            existing_numbers = [ann.get('number', 0) for ann in annotations if 'number' in ann]
            number = max(existing_numbers + [0]) + 1
            annotation['number'] = number
        else:
            number = annotation['number'] # Use existing number if present
    ...
    def zoom_image(self):
        zoom_factor = self.zoom_slider.value() / 100.0 # Ensure float division
    ...
    def finish_rectangle(self):
        if self.image_label.current_rectangle:
            ...
            new_annotation = {
                # Save rectangle as a polygon for consistency internally
                "segmentation": [x1, y1, x2, y1, x2, y2, x1, y2],
                # Also save the bbox explicitly if needed by export formats
                "bbox": [min(x1,x2), min(y1,y2), abs(x2-x1), abs(y2-y1)],
                "category_id": self.class_mapping[self.current_class],
                "category_name": self.current_class,
                "type": "rectangle" # Or keep as polygon if internal representation is always polygon
            }
            self.image_label.annotations.setdefault(self.current_class, []).append(new_annotation)
            self.add_annotation_to_list(new_annotation)
    ...
    def highlight_annotation_in_list(self, annotation):
        for i in range(self.annotation_list.count()):
            item = self.annotation_list.item(i)
            if item.data(Qt.ItemDataRole.UserRole) == annotation: # Use enum
...
################################################################
    ...
    def load_prediction_model(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted: # Use enum
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted: # Use enum
            try:
                ...
                self.start_training(epochs, imgsz)
                 ...
                 QMessageBox.critical(self, "Invalid Input", "Epochs and Image Size must be integers.")
    ...
    def initialize_yolo_trainer(self):
        if hasattr(self, 'current_project_dir'):
            ...
            print("Warning: Attempted to initialize YOLO trainer without a project directory.")
            # QMessageBox.warning(self, "No Project", "Please open or create a project first.")
    ...
    def training_finished(self, results):
        # Check if dialog still exists (user might close it)
        if hasattr(self, 'training_dialog') and self.training_dialog.isVisible():
            ...
            try:
                ...
                self.training_dialog.stop_signal.disconnect(self.yolo_trainer.stop_training_signal)
                 ...
                 # Signals might have already been disconnected
                 pass
    ...
    def set_confidence_threshold(self):
        ...
        new_threshold, ok = QInputDialog.getDouble(self, "Set Confidence Threshold",
                                                   ...
                                                   current_threshold, 0, 1, 2) # min, max, decimals
        if ok:
    ...
    def show_predict_dialog(self):
        ...
        for image_name in self.image_paths.keys():
            ...
            # Add checkbox to allow selection
            item.setFlags(item.flags() | Qt.ItemFlag.ItemIsUserCheckable) # Use enum
            item.setCheckState(Qt.CheckState.Unchecked) # Use enum
        ...
        button_box = QDialogButtonBox(QDialogButtonBox.StandardButton.Cancel) # Use enum
        predict_button = QPushButton("Predict")
        button_box.addButton(predict_button, QDialogButtonBox.ButtonRole.AcceptRole) # Use enum
        button_box.accepted.connect(dialog.accept)
        button_box.rejected.connect(dialog.reject)
        layout.addWidget(button_box)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted: # Use enum
             ...
             for i in range(image_list.count()):
                 ...
                 if item.checkState() == Qt.CheckState.Checked: # Use enum
    ...
    def run_predictions(self, selected_images):
        for image_name in selected_images:
            ...
            if image_path:
                 ...
                 print(f"Warning: Could not find path for image {image_name}")

    def process_yolo_results(self, results, image_name):
        image_path = self.image_paths.get(image_name)
        if not image_path:
            QMessageBox.warning(self, "Error", f"Could not find path for image: {image_name}")
            return
        # Use OpenCV to read image for dimensions, more robust for formats QImage might struggle with directly
        ...
        try:
            results_list, input_size, original_size = results  # Unpack the results, input size, and original size
            input_height, input_width = input_size # Might be different if model resizes
            orig_height, orig_width = original_size # From Ultralytics result object

            # Use original dimensions read by OpenCV if they match, otherwise prefer Ultralytics'
            if original_width != orig_width or original_height != orig_height:
                 print(f"Warning: Dimension mismatch between OpenCV ({original_width}x{original_height}) and Ultralytics ({orig_width}x{orig_height}). Using Ultralytics dimensions.")
            else:
                 orig_width = original_width
                 orig_height = original_height

            # Scaling factors remain the same conceptually
            scale_x = orig_width / input_width
            scale_y = orig_height / input_height

            for result in results_list: # Iterate through the list of results
                boxes = result.boxes
                masks = result.masks
                ...
                for mask, box in zip(masks.data, boxes): # Access mask data
                    try:
                        class_id = int(box.cls)
                        if self.yolo_trainer.class_names is None or class_id >= len(self.yolo_trainer.class_names):
                             QMessageBox.warning(self, "Class Name Error",
                                                 f"Cannot find class name for ID {class_id}. Ensure the correct YAML is loaded for prediction.")
                             continue # Skip this annotation
                        class_name = self.yolo_trainer.class_names[class_id]
                        score = float(box.conf)

                        # Mask data is usually relative to input size, needs scaling
                        mask_array = mask.cpu().numpy() # Get mask data as numpy array
                        # Resize mask to original image size
                        mask_array_resized = cv2.resize(mask_array, (orig_width, orig_height), interpolation=cv2.INTER_NEAREST) # Use nearest for masks

                        contours, _ = cv2.findContours((mask_array_resized > 0.5).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                        ...
                        if contours:
                            ...
                            polygon = contours[0].flatten().tolist() # Use raw contour

                            # No extra scaling needed if mask was resized to original dimensions
                            ...
                            temp_annotation = {
                                ...
                                "score": score,
                                "temp": True,
                                "type": "polygon" # Prediction result is a polygon
                            }
                        ...
                        return
                         ...
                         print(f"Error processing single prediction: {inner_e}")
                         continue # Skip problematic prediction

        except Exception as e:
            QMessageBox.warning(self, "Prediction Error",
                f"An error occurred during prediction processing: {str(e)}\n\n"
                "This might be due to a mismatch between the model and the YAML file classes, "
                "or an issue processing the prediction results. Please check console for details.")
        ...
        if temp_annotations:
            ...
            QMessageBox.information(self, "No Predictions",
                                    "No predictions were found for this image meeting the confidence threshold.")
                ...
                # Select first non-temp class if possible
                 ...
                 for i in range(self.class_list.count()):
                     ...
                     if not item.text().startswith("Temp-"):
                         ...
                         found_primary = True
                         break
                 if not found_primary: # If only temp classes exist
                     self.class_list.setCurrentRow(0)
                     self.on_class_selected(self.class_list.item(0))
            else:
                self.current_class = None
    ...
    def accept_visible_temp_classes(self):
        visible_temp_classes = [item.text() for i in range(self.class_list.count())
                                if self.class_list.item(i).text().startswith("Temp-") and
                                self.class_list.item(i).checkState() == Qt.CheckState.Checked] # Use enum

        for temp_class_name in visible_temp_classes:
            permanent_class_name = temp_class_name[5:]  # Remove "Temp-" prefix
            if permanent_class_name not in self.class_mapping:
            ...
            if temp_class_name in self.image_label.annotations: # Check if temp class still exists
                 for annotation in self.image_label.annotations[temp_class_name]:
                     current_max += 1
                     annotation['category_name'] = permanent_class_name
                     annotation['category_id'] = self.class_mapping[permanent_class_name] # Update category ID
                     annotation['number'] = current_max
                     del annotation['temp'] # Remove temp flag
                     if 'score' in annotation: del annotation['score'] # Remove score
        ...
        self.update_class_list()
        current_name = self.current_slice or self.image_file_name
        if current_name: # Ensure we have a valid key
            self.all_annotations[current_name] = self.image_label.annotations.copy() # Update main storage
        self.update_annotation_list()
        self.image_label.update()
        self.save_current_annotations()
        ...
        self.verify_current_class() # Ensure a valid class is selected
    ...
    def select_first_primary_class(self):
        for i in range(self.class_list.count()):
            ...
            if not item.text().startswith("Temp-"):
                self.class_list.setCurrentItem(item)
                self.on_class_selected(item)
                return # Exit after selecting the first primary class
        # If no primary class found, select the first item (which must be temp)
        if self.class_list.count() > 0:
             self.class_list.setCurrentRow(0)
             self.on_class_selected(self.class_list.item(0))
    ...
    def reject_visible_temp_classes(self):
        ...
        self.update_class_list()
        self.image_label.update()
        # Also update all_annotations if necessary (though temp annotations shouldn't be saved there yet)
        current_name = self.current_slice or self.image_file_name
        if current_name and current_name in self.all_annotations:
             for temp_class_name in visible_temp_classes:
                  if temp_class_name in self.all_annotations[current_name]:
                       del self.all_annotations[current_name][temp_class_name]

    def is_class_visible(self, class_name):
        items = self.class_list.findItems(class_name, Qt.MatchFlag.MatchExactly) # Use enum
        if items:
            return items[0].checkState() == Qt.CheckState.Checked # Use enum
        return False # Default to not visible if class not found in list
    ...
    def check_temp_annotations(self):
        ...
        if temp_classes:
            reply = QMessageBox.question(self, 'Temporary Annotations',
                                         ...
                                         QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) # Use enum
            if reply == QMessageBox.StandardButton.Yes: # Use enum
                for temp_class in list(temp_classes): # Iterate over a copy
                    if temp_class in self.image_label.annotations:
                        del self.image_label.annotations[temp_class]
                    if temp_class in self.image_label.class_colors:
    ...
    def remove_all_temp_annotations(self):
        for image_name in list(self.all_annotations.keys()):
            if isinstance(self.all_annotations[image_name], dict): # Ensure it's a dictionary
                ...
                if not self.all_annotations[image_name]: # Remove image entry if no classes left
